...
from src.models import BronzeStorageEntry, SilverStorageEntry
from src.components import Processor, SiteAParser
    ...
    for df in pdf:
        ...
        for _, row in df.iterrows():
            try:
                ...
                logger.error(f"Could not process row: {row.to_dict()}. Exception: {e}")
                continue
...
if total_rows > 0:
    processed_df = df_repart.mapInPandas(process_bronze_batch, schema=silver_schema_ddl)

    processed_df.write.format("delta").mode("overwrite").save(SILVER_DELTALAKE_LOCATION)
